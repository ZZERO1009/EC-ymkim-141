...
* @author  SSSLAB
* @Mod		 Youngmin 	
* @brief   Embedded Controller:  for pwm
* 
******************************************************************************
*/
...
// 2. Configure GPIO AFR by Pin num.	
	...
	// YOUR CODE GOES HERE
	// 0-0000 7-0111 /vs/ 8-1000 15-1111
...
// 4. Configure Timer Output mode as PWM
	...
	if(chN == 1){
		...
		TIMx->CCMR1 |= TIM_CCMR2_OC4M_1 | TIM_CCMR2_OC4M_2;															// OC1M = 110 for PWM Mode 1 output on ch3
		TIMx->CCMR1	|= TIM_CCMR2_OC4PE;                     					// Output 1 preload enable (make CCR3 value changable)	
...
// 3. Configure prescaler PSC
	...
	switch(chN){
		...
		case 2: TIMx->CCR1 = value; break;
		case 3: TIMx->CCR1 = value; break;
		case 4: TIMx->CCR1 = value; break;
		default: break;
	}
}
...
// 1. Configure prescaler PSC
	float value = (TIMx->ARR + 1) * duty;    								// (ARR+1)*dutyRatio + 1               
  	...
  	if(chN == 1)      { TIMx->CCR1 = value; }          //set channel      
		...
		else if(chN ==31)      { TIMx->CCR3 = value; }
