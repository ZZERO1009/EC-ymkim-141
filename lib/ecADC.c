 ...
 * @Created 2023-11-20
 * @Modified 2023-11-20
 ADC Configuration code for RC car
*/
...
// 1. Initialize GPIO port and pin as ANALOG, no pull up / pull down
	GPIO_init(port, pin, 3);  				// ANALOG = 3
	GPIO_pupd(port, pin, 0);  			// EC_NONE = 0
...
// 4. Single(one) Channel or Scan(multi-channel) mode  
	...
	ADC1->SQR1 &= ~(15<<ADC_SQR1_L_3); 					// 0000: one channel length in the regular channel conversion sequence
